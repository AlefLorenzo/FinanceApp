...
export function Sidebar({
  ...
  const wallets =
    useLiveQuery(() => db.wallets.toArray(), []) || [];

  const [isEditingBalance, setIsEditingBalance] =
    useState(false);
  ...
  const menuItems = [
    ...
    { id: 'reserves', label: 'Reservas', icon: LifeBuoy },
  ...
  const sidebarContent = (
    <div className="flex flex-col h-full bg-white border-r border-gray-100 p-6">
      ...
      <div className="flex justify-between items-center mb-8 shrink-0">
        ...
        <button
          onClick={onClose}
          className="md:hidden p-2 hover:bg-gray-50 rounded-xl transition"
        ...
        {isEditingBalance ? (
          ...
          >
            <MoneyInput
              autoFocus
              valueCents={
                parseBRLToCents(newBalance) ||
                summary.currentBalanceCents
              }
              onChangeCents={(cents) =>
                setNewBalance(cents.toString())
              }
            ...
            <button
              type="submit"
              className="text-green-600 bg-green-50 p-1.5 rounded-lg hover:bg-green-100"
            ...
            <button
              ...
              className="text-gray-500 bg-gray-100 p-1.5 rounded-lg hover:bg-gray-200"
              aria-label="Cancelar"
            >
              <X className="w-4 h-4" />
            </button>
          ...
          <button
            ...
            onClick={() => {
              setNewBalance(
                summary.currentBalanceCents.toString()
              );
              setIsEditingBalance(true);
            }}
            className="block w-full text-left text-xl font-black text-gray-950 truncate amount-text cursor-pointer hover:text-blue-600 transition"
